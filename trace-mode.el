...
;; A flexible monitoring mode for examining debug output.
;; Note that a set of C macros and routines exist which produce output
;; for this mode. 
...
(defsubst l-start () (save-excursion (beginning-of-line) (point)))
(defsubst l-end () (save-excursion (end-of-line) (point)))
  ...
  (let ((start (make-marker)) (end (make-marker)))
    ...
    (save-excursion
      (while (marker-position start)
        ...
        (let ((format-list trace-mode-format-list) (pos 0) sym)

          (while (and format-list (re-search-forward "\\w+" (l-end) t))

            (when (cadr format-list)
              ;; This one needs processing - get the symbol via symbol table.
              (setq sym (eval (intern (car (cddr format-list)) sym-table)))
              (setq sym (intern (match-string-no-properties 0) sym))
              (make-face sym)
              (add-text-properties (l-start) (l-end) (list 'face sym))
              (add-text-properties (l-start) 
                                   (save-excursion (forward-line 1) (point)) 
                                   (list 'invisible sym)))

            ;; After each field, we search ahead for the next and align it.
            (setq pos (+ 1 pos (car format-list)))
            (re-search-forward "[ \t]+" (l-end) t)
            (while (> (current-column) pos) (delete-char -1))
            (indent-to pos) 
            (setq format-list (nthcdr 3 format-list))))

        (set-marker start (text-property-any (point) (marker-position end) 
                                             'invisible nil)))))
...
(defun select-column ()
  (eval (intern-soft 
         (completing-read "Column to select from: " sym-table nil t) 
         sym-table))
)
