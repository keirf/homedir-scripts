...
(defcustom bk-binary
  (dolist (path '("/usr/groups/netos/ix86_linux/bitkeeper/bk"
		  "/net/bk/bin/bk"
		  "/usr/bin/bk"
		  "/usr/local/bin/bk"
  ...
  (if (fboundp 'replace-in-string)
	...
	(while (string-match regexp str start)
	  (setq mb (match-beginning 0)
		me (match-end 0)
		result (concat result (substring str start mb) newtext)
		start me))
...
(if (fboundp 'add-minor-mode)
  ...
  (or (assq 'bk-mode minor-mode-alist)
      (setq minor-mode-alist
            (cons '(bk-mode 'bk-mode) minor-mode-alist))))
