...
(defun conv-box-to-mini-comments-in-buffer ()
  "Scans current buffer for box comments that don't start on column 0.
Such comments are automatically converted to mini comments."
  (interactive)
  (save-excursion
    (beginning-of-buffer)
    (while (re-search-forward "^[ \t]+/\\*\\(\\*+/\\)[ \t]*$" (point-max) t)
      (let ((mb (match-beginning 1)))
        (when (eq (comment-type) 'solid)
          (delete-region mb (point))
          (forward-line 1)
          (while (< (point) ce-region)
            (limited-search-forward "/*")
            (forward-char -1)
            (delete-char -1)
            (insert-char 32 1)
            ...
            (forward-line 1))
          (forward-line -1)
          (when (limited-re-search-forward "\\*+")
...
(defun conv-to-new-comment-style ()
  ...
  (save-excursion
    ...
    (while (re-search-forward "^[ \t]*/\\*\\*+/$" (point-max) t)
      ...
      (insert "*")
      (forward-line 1)
      (while (limited-search-forward "/* ")
        (save-excursion (forward-char -2) (delete-char -1) (insert-char 32 1))
        (limited-search-forward "*/")
        (delete-char -2)
        (delete-horizontal-space)
        (forward-line 1))
      (beginning-of-line)
      (limited-re-search-forward "^[ \t]*\\(/\\*\\*+/\\)[ \t]*$")
